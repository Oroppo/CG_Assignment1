...
namespace Gameplay {
	...
	void Scene::SetColorLUT(const Texture3D::Sptr& texture, const int ID) {

		if (ID == 0) {
			_colorCorrection = texture;
		}
		else if (ID == 1) {
			_colorCorrection1 = texture;
		}
		else if (ID == 2) {
	...
	const Texture3D::Sptr& Scene::GetColorLUT() const {
		...
		else if (app.KeyPressed() == 9) {
			return _colorCorrection1;
		}
		else if (app.KeyPressed() == 0) {
