...
vec3 CalcPointLightContribution(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess, int key) {
	...
	if(key==1){
		...
		return specularOut + AmbientColAndNumLights.rgb;
	}
...
*/
vec3 CalcAllLightContribution(vec3 worldPos, vec3 normal, vec3 camPos, float shininess, int key) {
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		...
		lightAccumulation += CalcPointLightContribution(worldPos, normal, viewDir, Lights[ix], shininess, key);
	}
